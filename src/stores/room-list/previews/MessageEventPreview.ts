...
export class MessageEventPreview implements IPreview {
    public getTextFor(event: MatrixEvent, tagId?: TagID, isThread?: boolean): string | null {
        ...
        if (isSelf(event)) {
            // Tin nhắn của mình: luôn hiện "Bạn:"
            return _t("event_preview|m.text", { senderName: "Bạn", message: body });
        } else {
            // Tin nhắn của người khác
            if (shouldPrefix) {
                // Tin nhắn phòng: hiện tên người gửi
                return _t("event_preview|m.text", { senderName: getSenderName(event), message: body });
            } else {
                // Tin nhắn 1:1: chỉ hiện nội dung
                return body;
            }
        }
    }
